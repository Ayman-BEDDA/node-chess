...
const fetchData = async () => {
  try {
    ...
    if (responseUsers.ok) {
      users.push(...(await responseUsers.json()));
    ...
    if (responseArticles.ok) {
      articles.push(...(await responseArticles.json()));
      isLoading.value = false;
    } else {
    ...
    if (responseBuys.ok) {
        buys.push(...(await responseBuys.json()));
      isLoading.value = false;
    } else {
      alert('Error while fetching');
    }
    ...
    if (responseMoneys.ok) {
        moneys.push(...(await responseMoneys.json()));
      isLoading.value = false;
    } else {
      alert('Error while fetching');
    }
          ...
          <div class="projects">
            <div class="card">
              <div class="card-header">
                <h2>Les 10 derniers articles ajoutés</h2>
                <router-link to="/admin/articles" class="nav-link">Tout voir <span class="fas fa-arrow-right"></span></router-link>
              </div>
              <div class="card-body">
                <div class="table-responsive">
                  ...
                  <tbody>
                    <tr v-for="article in displayedArticles" :key="user.id">
          ...
          <div class="customers">
            <div class="card">
              <div class="card-header">
                  ...
                  <router-link to="/admin/users" class="nav-link">Tout voir <span class="fas fa-arrow-right"></span></router-link>
              </div>
